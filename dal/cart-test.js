const { CartItemTest } = require('../models');

async function getCartByUserId (userId) {
    const cart = await CartItemTest.collection().where({
        user_id:userId
    }).fetch({
        require:false,
        withRelated:['tea']
...
async function getCartItemByUserAndTeaId (userId,teaId){
    const cartItem = await CartItemTest.where({
        user_id:userId,
        tea_id:teaId
    }).fetch({
        require:false
 ...
 async function addOneCartItem (userId,teaId,quantity){
    ...
    if(cartItem == null){
        const newCartItem = new CartItemTest({
            user_id:userId,
            tea_id:teaId,
            quantity:quantity
        });
